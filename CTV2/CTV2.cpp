...
int lognin(void)
{
	...
	{		
		...
		if (first_name==CREATE)
		{
			//SetConsoleTextAttribute(hConsole, 15);
			addcustomer();
		}
		else if (first_name==GUEST)
		{
			//viewinventory();
		}
		else if (first_name!=GUEST&&first_name!= CREATE)
		{			
			//SetConsoleTextAttribute(hConsole, 1);
			cout << "\n\t\t";
			//SetConsoleTextAttribute(hConsole, 79);
...
int employee(int num)//Menu for employee accounts
{
	int choice,
		number=0;
	do 
